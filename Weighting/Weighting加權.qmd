...
多變項反覆加權需要使用`survey`套件，首先用`svydesign()`建立一的尚未加權的資料集，並計算出母體的期望值。

```{r}
SampleRK <- tscs201[, .(Sex, Age)]
Unweighted <- svydesign(ids = ~ 0, data = SampleRK)

sex2 <- Population[, .(人口數比例 = 人口數 / sum(人口數), Sex)][, .(Freq = nrow(SampleRK) * sum(人口數比例)), by = .(Sex)]
age2 <- Population[, .(人口數比例 = 人口數 / sum(人口數), Age)][, .(Freq = nrow(SampleRK) * sum(人口數比例)), by = .(Age)]
```

接著使用母體期望值進行加權，`sample.margins`為樣本資料的欄位，`population.margins`為母體資料的欄位。

```{r}
raking <- rake(design = Unweighted, sample.margins = list(~ Sex, ~ Age), 
               population.margins = list(sex2, age2))
SampleRK$Weight <- weights(raking)
```
...
```{r, warning=FALSE}
chisq.test(wpct(SampleRK$Sex, SampleRK$weights), p = as.numeric(sex[2, ]))
chisq.test(wpct(SampleRK$Age, SampleRK$weights), p = round(as.numeric(age[2, ]), 2))
```

透過卡方檢定的結果可以發現，無論是性別或是年齡組，p-value都大於0.05，樣本與母體沒有顯著差異，多變項反覆加權就正式完成了。
